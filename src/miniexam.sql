...
alter table agencies add check ( name notnull );
alter table agencies add check ( phone_number notnull );
...
select owners.first_name,house_type from owners join houses h on owners.id = h.owner_id
group by first_name,house_type order by length(owners.first_name) desc limit 1;
select case when price between 1500 and 2000 then 'True' else 'False' end as result from houses;
